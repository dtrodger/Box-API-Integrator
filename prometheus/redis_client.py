...
async def configure_redis_client(
    ...
    return await aioredis.create_redis_pool(
        f'redis://{config_file["redis_client"]["host"]}:{config_file["redis_client"]["port"]}',
        db=config_file["redis_client"]["db"],
        password=config_file["redis_client"]["password"],
    )
...
async def get(redis_client: aioredis.Redis, key: AnyStr) -> Optional[AnyStr]:
    ...
    try:
        ...
        log.error(f"get failed {str(e)} key {key}")
